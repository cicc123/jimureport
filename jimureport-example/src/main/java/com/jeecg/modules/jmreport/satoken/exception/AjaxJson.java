...
public class AjaxJson implements Serializable{
	...
	public static AjaxJson success(String msg, Object data) {
	...
	// 返回警告 
	public static AjaxJson warning() {
		return new AjaxJson(CODE_ERROR, "warning", null, null);
	}
	public static AjaxJson warning(String msg) {
		return new AjaxJson(CODE_WARNING, msg, null, null);
	}
	
	// 返回未登录
	public static AjaxJson notLogin() {
		return new AjaxJson(CODE_NOT_LOGIN, "未登录，请登录后再次访问", null, null);
	}
	
	// 返回没有权限的 
	public static AjaxJson notJur(String msg) {
	...
	// 返回分页和数据的
	public static AjaxJson pageData(Long dataCount, Object data){
		return new AjaxJson(CODE_SUCCESS, "ok", data, dataCount);
	}
	
	// 返回，根据受影响行数的(大于0=ok，小于0=error)
	public static AjaxJson byLine(int line){
		if(line > 0){
			return success("ok", line);
		}
		return error("error").setData(line); 
	}

	// 返回，根据布尔值来确定最终结果的  (true=ok，false=error)
	public static AjaxJson byBoolean(boolean b){
